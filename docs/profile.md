...
- `CSV_FILE`, `N_COMBINATIONS`, `VALID_FREQS`, `RUNS`, `TIMEOUT`, and
  `SLEEP_TIMES` in `configs/profiling/shared/config*.sh`;
...
them from `runners/shared`. The baseline scripts use paths relative to
`runners/baseline`, so invoke them from that directory.
...
colocated execution logs
  -> data/colocations/*.csv
baseline features + stage2 labels
  -> data/model_datasets/*_total_labels_comb{2,3}.csv
training and prediction
  -> artifacts/predictions/.../predicts_by_workloads/*.csv
paper result parser
  -> summary CSVs
```
...
```bash
cd "$PEACE_ROOT/runners/baseline"

# Edit VALID_FREQS and DEVICE_NUM in baseline_freqscale.sh first.
bash "$PEACE_ROOT/runners/baseline/baseline_freqscale.sh" \
...
bash "$PEACE_ROOT/runners/baseline/baseline_freqscale_customscript.sh" \
...
```bash
cd "$PEACE_ROOT/data/baseline_metrics"
...
`scripts/datasets/get_all_combinations.py` with the single-workload
baseline metrics produced in Section 1:

```text
python "$PEACE_ROOT/scripts/datasets/get_all_combinations.py" \
  --baseline_file BASELINE_METRICS.csv \
  --num_combinations N \
  --output_prefix OUTPUT_PREFIX
```

Run the command from the desired manifest subdirectory because the generated
CSV is written to the current directory. For example, given a 300 MHz baseline file
containing the 11 batch-2 DL workloads:

```bash
cd "$PEACE_ROOT/data/experiment_inputs/colocations/comb2"

python "$PEACE_ROOT/scripts/datasets/get_all_combinations.py" \
  --baseline_file $PEACE_ROOT/data/baseline_metrics/0506_FREQ300_DL_baseline_metrics.csv \
  --num_combinations 2 \
...
`data/experiment_inputs/colocations/comb2/05052025_freq300_DL_baseline_labels_comb2_batches2.csv`.
...
```bash
# Example settings in configs/profiling/shared/config.sh
CSV_FILE="$PEACE_ROOT/data/experiment_inputs/colocations/comb2/05052025_freq300_DL_baseline_labels_comb2_batches2.csv"
N_COMBINATIONS=2
```
...
```bash
cd "$PEACE_ROOT/runners/shared"

# Edit config.sh: CSV_FILE, N_COMBINATIONS, VALID_FREQS, and GPU settings.
bash "$PEACE_ROOT/runners/shared/auto_multiinstance_freqscale.sh" \
...
bash "$PEACE_ROOT/runners/shared/auto_multiinstance_nonDL_freqscale.sh" \
...
bash "$PEACE_ROOT/runners/shared/auto_multiinstance_freqscale_gpumonitor_fixedpower.sh" \
...
bash "$PEACE_ROOT/runners/shared/auto_multiinstance_nonDL_freqscale_gpumonitor_fixedpower.sh" \
...
```text
python "$PEACE_ROOT/scripts/parsing/colocations/stage2.py" INPUT_LOG_DIR OUTPUT_PREFIX NUM_COMBINATIONS [METRIC_TYPE]
```
...
from `data/colocations`. It writes its CSVs to that same directory.
...
python "$PEACE_ROOT/scripts/parsing/colocations/stage2.py" \
  ../../ccv100_logs/colocated_DL_nodvfs \
  05052025_freq300 \
  2 \
  throughput
...
`data/model_datasets/05052025_FREQ300_*`:

```bash
cd "$PEACE_ROOT"

python main.py -pd2 \
  -sd data/colocations/05052025_nonDL_freq300_nodvfs_share_comb2_freqscale_throughput_individual_avg.csv \
  -power data/colocations/05052025_nonDL_freq300_nodvfs_share_comb2_freqscale_power_avg_stage2.csv \
  -duration data/colocations/05052025_nonDL_freq300_nodvfs_share_comb2_freqscale_duration_avg_stage2.csv \
  -energy data/colocations/05052025_nonDL_freq300_nodvfs_share_comb2_freqscale_energy_avg_stage2.csv \
  -comb 2 \
  -t 100 \
  -mt threadclass \
  --output_prefix 0505_FREQ300_nodvfs \
  --output_dir data/model_datasets/05052025_FREQ300_nonDL_nodvfs \
  --label_policy separate_throughputpower_regression \
  --baseline_file data/baseline_metrics/0506_FREQ300_baseline_metrics.csv
```
...
```bash
python main.py -pd2 \
  -sd data/colocations/05052025_powercap60_DL_dvfs_share_comb2_freqscale_throughput_individual_avg.csv \
  -power data/colocations/05052025_powercap60_DL_dvfs_share_comb2_freqscale_power_avg_stage2.csv \
  -duration data/colocations/05052025_powercap60_DL_dvfs_share_comb2_freqscale_duration_avg_stage2.csv \
  -energy data/colocations/05052025_powercap60_DL_dvfs_share_comb2_freqscale_energy_avg_stage2.csv \
  -comb 2 \
  -t 100 \
  -mt threadclass \
  --output_prefix 0505_FREQ300_dvfs \
  --output_dir data/model_datasets/05052025_FREQ300_DL_powercap60_dvfs \
  --label_policy separate_throughputpower_regression \
  --baseline_file data/baseline_metrics/0506_FREQ300_baseline_metrics.csv
```
...
```bash
python main.py -pd2 \
  -sd data/colocations/09152025_freq300_DL_comb3_share_comb3_freqscale_throughput_individual_avg.csv \
  -power data/colocations/09152025_freq300_DL_comb3_share_comb3_freqscale_power_avg_stage2.csv \
  -duration data/colocations/09152025_freq300_DL_comb3_share_comb3_freqscale_duration_avg_stage2.csv \
  -energy data/colocations/09152025_freq300_DL_comb3_share_comb3_freqscale_energy_avg_stage2.csv \
  -comb 3 \
  -t 100 \
  -mt threadclass \
  --output_prefix 09152025_freq300_DL_comb3 \
  --output_dir data/model_datasets/09152025_freq300_DL_comb3 \
  --label_policy separate_throughputpower_regression \
  --baseline_file data/baseline_metrics/0506_FREQ300_baseline_metrics.csv
```
...
path = "data/model_datasets/09152025_freq300_DL_comb3/09152025_freq300_DL_comb3_throughput_total_labels_comb3.csv"
df = pd.read_csv(path)
...
dates. `data/model_datasets/merge_cudaDL.py` contains that historical
...
Edit `runners/training/run_unseen_partitions.sh`:
...
The wrapper resolves its worker script from `runners/training`, so it can be
invoked from the repository root:

```bash
cd "$PEACE_ROOT"
bash "$PEACE_ROOT/runners/training/run_unseen_partitions.sh"
```

For combination 3, run once per fixed-frequency dataset with output roots:

```text
$PEACE_ROOT/artifacts/predictions/09152025_freq300_DL_comb3
$PEACE_ROOT/artifacts/predictions/09152025_freq900_DL_comb3
$PEACE_ROOT/artifacts/predictions/09152025_freq1530_DL_comb3
```
...
```bash
cd "$PEACE_ROOT"
bash "$PEACE_ROOT/runners/training/run_train_splits_mpsthreads_crossvalidate.sh"
```
...
```text
artifacts/predictions/<experiment>/seen_partition/crossvalid/
...
for freq in 300 900 1530; do
  root="artifacts/predictions/09152025_freq${freq}_DL_comb3"
