...
root_dataset_output_dir="$REPO_ROOT/data/model_datasets/03112025_DL0207_0307_nonDL0311_nodvfs/seen_partition"
...
for n_occur in "${n_occurs[@]}"; do
    ...
    #bash "$SCRIPT_DIR/train_splits.sh" KACE  0 $n_occur output/run0730_KACE_batch2  train predict
    ...
    bash "$SCRIPT_DIR/train_splits_mpsthreads_rdseed.sh" extratrees 0 $n_occur "$output_result_dir"  train predict_all_throughput "${datapath}" "${root_dataset_output_dir}" "${rm_100partitions}" "${n_combinations}" throughput
   # bash "$SCRIPT_DIR/train_splits_mpsthreads_rdseed.sh" AutoML 0 $n_occur output/test_run03072025_data1229_mergecudaDL_comb2  train predict_all_throughput "${datapath}" "${root_dataset_output_dir}" "${rm_100partitions}" "${n_combinations}" throughput
    ...
    #bash "$SCRIPT_DIR/train_splits.sh" NN 0 $n_occur output/run0730_KACE_batch2-8  wefhuehf predict
    #bash "$SCRIPT_DIR/train_splits.sh" RF 0 $n_occur output/run0730_KACE_batch2-8  fsefe predict
