...
source "$REPO_ROOT/configs/profiling/shared/config_nonDL_gpumonitor_missingthreads.sh"
...
while IFS=, read -ra columns; do
    ...
    # Check if powercap value is valid
    if [[ -z "$powercap_val" ]] || ! [[ "$powercap_val" =~ ^[0-9]*\.?[0-9]+$ ]]; then
        echo "PowerCap value missing or invalid: $powercap_val"
        continue  # Skip if PowerCap is invalid
    fi

    # Calculate threshold powercap value
    powercap_val_thres=$(echo "$powercap_val * $GPU_POWERCAP_THRES" | bc -l)
    ...
    for RUN in $(seq 1 $RUNS); do
        ...
        for SLEEP_TIME in "${SLEEP_TIMES[@]}"; do
            ...
            python "$REPO_ROOT/scripts/profiling/GPUpowerMonitor.py" \
                --logfile "$MPS_LOG_DIR/gpu_monitor_FREQ0_${w1_percentage}_${w2_percentage}_SLEEP${SLEEP_TIME}.log" \
            ...
            nvidia-smi pmon -o DT -i "$GPUMON_GPU_INDEX" \
                -f "$MPS_LOG_DIR/gpu_info_FREQ0_${w1_percentage}_${w2_percentage}_SLEEP${SLEEP_TIME}.csv" &
            nvidia_smi_pid=$!
            ...
            nvidia-smi -i "$GPUMON_GPU_INDEX" --query-gpu=timestamp,name,pci.bus_id,driver_version,pstate,pcie.link.gen.max,pcie.link.gen.current,memory.total,memory.free,memory.used,power.draw,power.limit,clocks.gr,clocks.sm,clocks.mem --format=csv --loop-ms=500 \
                > "$MPS_LOG_DIR/gpu_mem_FREQ0_${w1_percentage}_${w2_percentage}_SLEEP${SLEEP_TIME}.csv" &
            nvidia_smi_pid2=$!

            dcgmi dmon -e 1002,1003,1004,1005,1006,1007,1008,1009,1010,1011,1012 -i "$GPUMON_GPU_INDEX" \
                > "$MPS_LOG_DIR/dcgm_info_FREQ0_${w1_percentage}_${w2_percentage}_SLEEP${SLEEP_TIME}.csv" &
            dcgmi_pid=$!
