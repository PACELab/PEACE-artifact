...
train_candidates=(
    ...
    "cuda_samples:BlackScholes"
    "cuda_samples:sortingNetworks"
    "cuda_samples:transpose"
    "cuda_samples:reductionMultiBlockCG"
    "cuda_samples:fastWalshTransform"
    "cuda_samples:cudaTensorCoreGemm"
...
BE_PERCENTAGES=(100 20 10 50 90 80 70 60 40 30)
#LS_PERCENTAGES=(0)
#BE_PERCENTAGES=(20)
RUNS=1
            ...
            for TASK_MODEL in "${TASKS_MODELS[@]}"; do
                ...
                if [ $LS_PERCENTAGE -eq 0 ]; then
                    ...
                    if [ "$LS_MODE" == "train" ]; then
                        ...
                        --env NVIDIA_VISIBLE_DEVICES=$DEVICE_NUM --gpus device=$DEVICE_NUM \
                        ...
                        --batch_size ${LS_BATCH} --log_dir ../../$CURR_LOG_DIR"  \
                        > $CURR_LOG_DIR/LS_${LS_TASK}_MPS${LS_PERCENTAGE}.log 2>&1 & ls_pid=$!
                    else
                        docker run --rm --name LS --env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=$LS_PERCENTAGE \
                        --env NVIDIA_VISIBLE_DEVICES=1 --gpus device=$DEVICE_NUM \
                        -v /tmp/nvidia-mps:/tmp/nvidia-mps \
                        -v ~/.cache/huggingface:/root/.cache/huggingface \
                        -v ~/.cache/torch:/root/.cache/torch \
                        -v "$REPO_ROOT:/root/mlprofiler" --ipc=host \
                        ...
                        --batch_size ${LS_BATCH} --log_dir ../../$CURR_LOG_DIR --profile_nstep 200"  \
                        > $CURR_LOG_DIR/LS_${LS_TASK}_MPS${LS_PERCENTAGE}.log 2>&1 & ls_pid=$!
                    fi
                fi
                ...
                if [ "$BE_MODE" == "train" ]; then
                    ...
                    --cap-add=SYS_ADMIN -v /opt/nvidia/nsight-systems/2023.3.3:/nsys \
                    nba556677/ml_tasks:cuda_samples  \
                    /bin/sh -c "cd /root/mlprofiler/workloads/cuda_samples/benchmarks/${MODEL}; ./${MODEL} " \
                    | stdbuf -oL tee $CURR_LOG_DIR/BE_${MODEL}_MPS${BE_PERCENTAGE}.log  2>&1 & be_pid=$!


                    #echo "start script..."
                    #add cuurent pwd to CURR_LOG_DIR
                    #CURR_LOG_DIR=$(pwd)/$CURR_LOG_DIR
                    #cd ~/geepafs/cuda_samples/benchmarks/BlackScholes
                    #echo "curr log dir: $CURR_LOG_DIR"
                    #NVIDIA_VISIBLE_DEVICES=1  CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=$BE_PERCENTAGE ./"$MODEL"  > $CURR_LOG_DIR/BE_${MODEL}_MPS${BE_PERCENTAGE}.log 2>&1 & be_pid=$!
                    #echo "BlackScholes PID: $be_pid"
                else
                    ...
                    --batch_size ${BE_BATCH} --log_dir ../../$CURR_LOG_DIR --profile_nstep 100"  \
                ...
                nvidia-smi pmon -o DT -i $DEVICE_NUM > $CURR_LOG_DIR/BE_${TASK}_MPS${BE_PERCENTAGE}_gpu_info.csv &
                nvidia-smi -i $DEVICE_NUM --query-gpu=timestamp,name,pci.bus_id,driver_version,pstate,pcie.link.gen.max,pcie.link.gen.current,memory.total,memory.free,memory.used,power.draw,power.limit --format=csv --loop-ms=500  > $CURR_LOG_DIR/BE_${TASK}_MPS${BE_PERCENTAGE}_gpu_mem.csv &
                dcgmi dmon -e 1002,1003,1004,1005,1006,1007,1008,1009,1010,1011,1012  -i $DEVICE_NUM  > $CURR_LOG_DIR/BE_${TASK}_MPS${BE_PERCENTAGE}_dcgm_info.csv & 
