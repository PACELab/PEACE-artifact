...
##REMEMBER to export HF_TOKEN="your_hf_token_here" in terminal first!!!
...
TASKS_MODELS=()
#TODO - replace batch size with output length
batch_sizes=(20)
...
inference_candidates=(
    "autoregressive:gpt2-xl"
    #"autoregressive:google/gemma-3-1b-it" # Added Gemma. 0531 - not working yet
    ...
    for LS_MODEL in "${LS_MODELS[@]}"; do
        ...
        for ((i=0; i<${#LS_PERCENTAGES[@]}; i++)); do
            ...
            for TASK_MODEL in "${TASKS_MODELS[@]}"; do
                IFS=':' read -r TASK MODEL BE_EPOCH BE_OUTPUTLEN <<< "$TASK_MODEL"
                echo "TASK=$TASK, MODEL=$MODEL BE_EPOCH=$BE_EPOCH BATCH=1  OUTPUTLEN=$BE_OUTPUTLEN"

                for FREQ_SCALING in "${VALID_FREQS[@]}"; do
                    echo "Running with frequency scaling: $FREQ_SCALING MHz"
                    CURR_LOG_DIR="$LOG_DIR/FREQ${FREQ_SCALING}/RUN${RUN}/LS${LS_PERCENTAGE}/${LS_TASK}/${LS_MODEL}_batch${LS_BATCH}/BE_${MODEL}_batch${BE_OUTPUTLEN}"
                    ...
                    if [ $LS_PERCENTAGE -eq 0 ]; then
                        ...
                        if [ "$LS_MODE" == "train" ]; then
                            ...
                            --batch_size ${LS_BATCH} --log_dir ../../$CURR_LOG_DIR --profile_nstep 100"  \
                            > $CURR_LOG_DIR/LS_${LS_TASK}_FREQ${FREQ_SCALING}_MPS${LS_PERCENTAGE}.log 2>&1 & pids+=($!)
                        fi
                    fi
                    ...
                    if [ "$BE_MODE" == "train" ]; then
                        ...
                        --batch_size 1 --log_dir ../../$CURR_LOG_DIR --output_length ${BE_OUTPUTLEN} --num_requests 1000000000 --profile_nstep 100"  \
                    ...
                    nvidia-smi pmon -o DT -i $DEVICE_NUM > $CURR_LOG_DIR/BE_${TASK}_FREQ${FREQ_SCALING}_MPS${BE_PERCENTAGE}_gpu_info.csv &
                    nvidia-smi -i $DEVICE_NUM --query-gpu=timestamp,name,pci.bus_id,driver_version,pstate,pcie.link.gen.max,pcie.link.gen.current,memory.total,memory.free,memory.used,power.draw,power.limit --format=csv --loop-ms=500  > $CURR_LOG_DIR/BE_${TASK}_FREQ${FREQ_SCALING}_MPS${BE_PERCENTAGE}_gpu_mem.csv &
                    dcgmi dmon -e 1002,1003,1004,1005,1006,1007,1008,1009,1010,1011,1012  -i 1 | awk '{print strftime("%Y-%m-%d %H:%M:%S"), $0}' > $CURR_LOG_DIR/BE_${TASK}_FREQ${FREQ_SCALING}_MPS${BE_PERCENTAGE}_dcgm_info.csv & 
