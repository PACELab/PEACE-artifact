            ...
            for TASK_MODEL in "${TASKS_MODELS[@]}"; do
                ...
                for FREQ_SCALING in "${VALID_FREQS[@]}"; do
                    ...
                    if [ "$BE_MODE" == "train" ]; then
                        ...
                        --model_name ${MODEL} --batch_size ${BE_BATCH} --profile_nstep 9999999" \
                        ...
                        --batch_size ${BE_BATCH} --log_dir ../../$CURR_LOG_DIR --profile_nstep 9999999"  \
                        > $CURR_LOG_DIR/BE_${TASK}_FREQ${FREQ_SCALING}_MPS${BE_PERCENTAGE}.log 2>&1 & pids+=($!)
                    fi
                    
                    #nvidia-smi pmon -o DT -i 1 -f $CURR_LOG_DIR/BE_${TASK}_MPS${BE_PERCENTAGE}_gpu_info.csv &
                    nvidia-smi pmon -o DT -i 1 > $CURR_LOG_DIR/BE_${TASK}_FREQ${FREQ_SCALING}_MPS${BE_PERCENTAGE}_gpu_info.csv &
                    nvidia-smi -i 1 --query-gpu=timestamp,name,pci.bus_id,driver_version,pstate,pcie.link.gen.max,pcie.link.gen.current,memory.total,memory.free,memory.used,power.draw,power.limit --format=csv --loop-ms=500  > $CURR_LOG_DIR/BE_${TASK}_FREQ${FREQ_SCALING}_MPS${BE_PERCENTAGE}_gpu_mem.csv &
