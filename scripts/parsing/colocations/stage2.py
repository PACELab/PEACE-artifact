...
PARSING_DIR = Path(__file__).resolve().parent.parent
REPO_ROOT = PARSING_DIR.parent.parent
COLOCATIONS_DIR = REPO_ROOT / "data" / "colocations"
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        ...
        exit(1)
    base_df = pd.read_csv(COLOCATIONS_DIR / "baseline_steps_stage2.csv")
    share_directory_path = str(Path(sys.argv[1]).resolve())
    output_prefix = sys.argv[2]
    N_COMB=int(sys.argv[3])
    # Write all parsed colocation outputs to the canonical data directory.
    os.chdir(COLOCATIONS_DIR)
    ...
    #share_directory_path = "/home/cc/mlProfiler/data/experiment_inputs/colocations/ccv100/comb4_testbatch4_trainbatch2-8"
