...
def get_pred_oracle_target_comparison(pred_df, pred_power_df, all_oracle_df, powercap_limit_df, target, static_power_limit, weight, output_dir,rm_100partition, power_threshold=0.75, is_plot=False):
    ...
    elif target == "max_xput" or target == "xput_under_powercap":
        all_oracle_df[target] = all_oracle_df["weight_Throughput_sum"]
        all_df[target] = all_df["weight_Throughput_sum"]
    ...
    #record pairs with valid prediction pairs
    ...
    for workload1, workload2 in workload_pairs:
        ...
        if target in ["EDP", "energy_plus_duration", "power_per_xput"]:
        ...
        elif target in ["xput_per_joule", "xput_per_power", "max_xput"]:
            #get max target idx
            ...
            # Oracle: Select max throughput where actual power is under static_power_limit
            if static_power_limit is None:
                raise ValueError("static_power_limit is None, cannot filter by power cap!")
            valid_oracle_df = filtered_all_df[filtered_all_df["Power"] <= static_power_limit]
            if valid_oracle_df.empty:
                print(f"No oracle configurations under power cap {static_power_limit} for {workload1}, {workload2}")
                select_idx = None
            else:
                select_idx = valid_oracle_df[target].idxmax()
            
            # Predicted: Select max predicted throughput where predicted power is under static_power_limit
            valid_pred_df = filtered_all_df[filtered_all_df["y_pred_power"] <= static_power_limit]
            if valid_pred_df.empty:
                print(f"No predicted configurations under power cap {static_power_limit} for {workload1}, {workload2}")
                select_pred_idx = None
            else:
                select_pred_idx = valid_pred_df[f"pred_{target}"].idxmax()

            #save valid_oracle_df and valid_pred_df to csv
            valid_oracle_df.to_csv(f"{output_dir}/{workload1}_{workload2}_filtered_oracle_df.csv", index=False)
            valid_pred_df.to_csv(f"{output_dir}/{workload1}_{workload2}_filtered_pred_df.csv", index=False)

        else:
            print(f'is {target in ["EDP", "energy_plus_duration", "xput_per_joule", "xput_per_power"]}')
            raise ValueError(f"Unsupported target value {target}")
        
        # Skip if no valid configurations found
        if select_idx is None or select_pred_idx is None:
            if select_idx is not None and select_pred_idx is None:
    ...
    if final_df.empty:
        ...
        return final_df, all_df, num_invalid_pre_configs
    ...
    for root, _, files in os.walk(predict_xput_dir):
        ...
        if pred_xput_file_common_name in files:
            ...
            if os.path.exists(power_file):
                ...
                final_df, all_df, num_invalid_prediction_configs = get_pred_oracle_target_comparison(pred_df, pred_power_df, all_dvfs_df, powercap_limit_df, 
...
def analyze_predictions_target_with_baselines(predict_xput_dir, predict_power_dir, dvfs_file, temporal_file, powercap_limit_file, static_power_limit, pred_xput_file_common_name, pred_power_file_common_name,  target, output_base_name, weight, dvfs_oracle, rm_100partition, is_plot=False, save_csv=True):
    all_dvfs_df = pd.read_csv(dvfs_file)
    if temporal_file:
        temporal_df = pd.read_csv(temporal_file) 
    else:
    ...
    if powercap_limit_file:
        ...
        powercap_limit_df = None
    #check if static powerlmit is interger
    if not isinstance(static_power_limit, (int, float)):
        static_power_limit = None
    all_final_dfs = []
    all_baseline_gains = []
    all_invald_pred_pairs = []
    output_dir = os.path.dirname(output_base_name)
    ...
    for root, _, files in os.walk(predict_xput_dir):
        ...
        if pred_xput_file_common_name in files:
            ...
            if os.path.exists(power_file):
                ...
                final_df, all_df, num_invalid_pred_pairs = get_pred_oracle_target_comparison(pred_df, pred_power_df, all_dvfs_df, powercap_limit_df, target, static_power_limit=static_power_limit ,weight=weight, output_dir=output_base_name, rm_100partition=rm_100partition,  is_plot=is_plot,
                                            )

                if not final_df.empty:
                    all_final_dfs.append(final_df)
                    all_invald_pred_pairs.append((power_root, num_invalid_pred_pairs))
    ...
    mean_ratios.to_csv(f"{output_base_name}_mean_ratios.csv", index=True)
    print(f"Mean ratios:\n{mean_ratios}")
    #print(f"invalid pred pairs:\n{all_invald_pred_pairs}")
    #save invalud pred pairs to csv
    all_invald_pred_pairs_df = pd.DataFrame(all_invald_pred_pairs)
    all_invald_pred_pairs_df.to_csv(f"{output_base_name}_invalid_pred_pairs.csv", index=False)


if __name__ == "__main__":

    #predict_xput_dir = "/Users/bing/Documents/Documents - Bing’s MacBook Air/mlProfiler/artifacts/predictions/05052025_FREQ900_nodvfs_fullDL/unseen_partition/throughput/rand10/extratrees"
    #predict_power_dir = "/Users/bing/Documents/Documents - Bing’s MacBook Air/mlProfiler/artifacts/predictions/05052025_FREQ900_nodvfs_fullDL/unseen_partition/power/rand10/extratrees"
    
    #FREQ1530- use aicrossroad files
    ...
    oracle_file = "/Users/bing/Documents/Documents - Bing’s MacBook Air/mlProfiler/data/model_datasets/05052025_FREQ300_DL_powercap60_dvfs/0505_FREQ300_dvfs_throughput_total_labels_comb2_labels.csv"
    ...
    #target = "xput_under_powercap" #EDP, xput_per_joule, xput_under_powercap
    target = "max_xput"
    weight = 1# convert to kj if weight=1000
    is_plot = False #plot each workload
    dvfs_oracle = True #whether oracle is dvfs enabled
    output_base_dir = f"./{target}_unseen_freq1530_powercap60_pred_vs_baselines_dvfs/"
    rm_100partition=True #remove 100 partition
    save_csv = True
    power_epsilon = 10
    #static power limit
    static_power_limit = 60 + power_epsilon
    ...
    #                                          temporal_file=temporal_file, powercap_limit_file=powercap_limit_100_file, static_power_limit=static_power_limit,
    ...
    #freq900
    predict_xput_dir = "/Users/bing/Documents/Documents - Bing’s MacBook Air/mlProfiler/artifacts/predictions/05052025_FREQ900_mergecudaDL_nodvfs_remerge/seen_partition/crossvalid/throughput/trainratio_/rand10"
    predict_power_dir = "/Users/bing/Documents/Documents - Bing’s MacBook Air/mlProfiler/artifacts/predictions/05052025_FREQ900_mergecudaDL_nodvfs_remerge/seen_partition/crossvalid/power/trainratio_/rand10"
    oracle_file = "/Users/bing/Documents/Documents - Bing’s MacBook Air/mlProfiler/data/model_datasets/05052025_FREQ900_mergecudaDL_nodvfs_remerge/merged_labels.csv"
    #freq300
    predict_xput_dir = "/Users/bing/Documents/Documents - Bing’s MacBook Air/mlProfiler/artifacts/predictions/05052025_FREQ300_mergecudaDL_nodvfs_remerge/seen_partition/crossvalid/throughput/trainratio_/rand10"
    predict_power_dir = "/Users/bing/Documents/Documents - Bing’s MacBook Air/mlProfiler/artifacts/predictions/05052025_FREQ300_mergecudaDL_nodvfs_remerge/seen_partition/crossvalid/power/trainratio_/rand10"
    oracle_file = "/Users/bing/Documents/Documents - Bing’s MacBook Air/mlProfiler/data/model_datasets/05052025_FREQ300_mergecudaDL_nodvfs_remerge/merged_labels.csv"
    ...
    temporal_file = None
    powercap_limit_100_file = None
    target = "max_xput" #EDP, xput_per_joule
    weight = 1# convert to kj if weight=1000
    is_plot = False #plot each workload
    ...
    all_models = ["RF", "AutoML", "extratrees"]
