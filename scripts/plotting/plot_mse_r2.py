...
def plot_unseen_data(avg_data, output_dir, baselines):
    ...
    #set title
    axs.set_title('4-workloads unseen', fontsize=label_font+2)
    plt.tight_layout()
...
# Base directory where your output is located
#python plot_mse_r2.py /Users/bing/Documents/mlProfiler/data/experiment_inputs/colocations/output/run0730_KACE_batch2-8/
#error check args
if len(sys.argv) < 2:
...
plot_unseen_data(avg_unseen_data, f"{base_dir}", baselines=[ 'Oracle','LR',"Best-rule-based", "memcap", "sm%", "mem%","Random" , "exclusive_throughput", "_Compute (SM) Throughput"])