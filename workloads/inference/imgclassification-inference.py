...
def inference(args: argparse.Namespace, profiler: MLProfiler = None) -> None:
    ...
    warmup = 10
    steps = 0 #total steps runned

    idx = 0  # Start index for dataset iteration
    dataset_length = len(testing_dataset)
    # inference the testing dataset
    while True:
        if idx >= dataset_length:
            idx = 0  # Restart dataset iteration if exhausted
        ...
        logger.log(f"Processing data = data {idx} to {idx + args.batch_size}")
        if steps >= warmup:  
            if args.profile  :
                torch.cuda.nvtx.range_push(f"moveData{steps}")
        batch = testing_dataset[idx:idx + args.batch_size]
