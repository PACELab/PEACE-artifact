...
def getMemStats(ts, state):
    ...
    mem_dict[ts] = {'allocated' :  allocated, 'reserved' : reserved, 'state' : state}
...
def main():
    ...
    global_train_start_time = time()
    getMemStats(ts=time() - global_train_start_time, state='load_model')
    #print(mem_dict)
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            getMemStats(ts=time() - global_train_start_time, state='fwd')
            loss = outputs.loss
            ...
            getMemStats(ts=time() - global_train_start_time, state='bwd')
            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                optimizer.step()
                print("after optimizer_step")
                getMemStats(ts=time() - global_train_start_time, state='optimizer.step')
                lr_scheduler.step()
                optimizer.zero_grad()
