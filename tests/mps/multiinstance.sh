...
#LS_PERCENTAGES=(100 10 20 30 40 50 60 70 80 90)
...
LS_PERCENTAGES=(70 20)
BE_PERCENTAGES=(30 80)
RUNS=1
BE_INSTANCES=9
    ...
    for LS_MODEL in "${LS_MODELS[@]}"; do
        ...
        for ((i=0; i<${#LS_PERCENTAGES[@]}; i++)); do
            ...
            for TASK_MODEL in "${TASKS_MODELS[@]}"; do
                ...
                for j in $(seq 1 $BE_INSTANCES); do
                    echo "starting BE intance $j..."
                
                    #BEMODE = train
                    if [ "$BE_MODE" == "train" ]; then
                        echo "BE_MODE=train"
                        # Start BE job
                        docker run --rm --name BE$j --env CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=$BE_PERCENTAGE \
                        --env NVIDIA_VISIBLE_DEVICES=1 --gpus device=1 \
                        -v /tmp/nvidia-mps:/tmp/nvidia-mps \
                        -v ~/.cache/huggingface:/root/.cache/huggingface \
                        -v ~/.cache/torch:/root/.cache/torch \
                        ...
                        > $CURR_LOG_DIR/BE_${TASK}_${j}_MPS${BE_PERCENTAGE}.log 2>&1  & be_pid=$!
                    else
                        ...
                        --batch_size ${BE_BATCH} --log_dir ../../tests/mps/$CURR_LOG_DIR --profile_nstep 500"  \
                        > $CURR_LOG_DIR/BE_${TASK}_${j}_MPS${BE_PERCENTAGE}.log 2>&1 & be_pid=$!
                    fi
                ...
                # Terminate client
                #terminate with LS and BE_INSTANCES
