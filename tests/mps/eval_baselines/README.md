...
```text
tests/mps/freq_scaling/baseline_metrics/0506_FREQ300_baseline_metrics.csv
tests/mps/freq_scaling/baseline_metrics/0502_FREQ900_baseline_metrics.csv
tests/mps/freq_scaling/baseline_metrics/0206_FREQ1530_baseline_metrics.csv
```

They must contain `Type`, `freq`, `Exclusive10` through `Exclusive100`, and
`SMACT%100`. Generate them by following Section 1 of `doc/profile.md`.
...
and `stage2.py` parsing steps in Sections 2 and 3 of `doc/profile.md`.
...
All paths in the table are relative to `tests/mps/analysis/stage2/`.
...
```bash
python tests/mps/eval_baselines/mudi/curvefit/kneedle_curvefit.py \
  --input_csv \
    tests/mps/analysis/stage2/05052025_freq300_share_comb2_freqscale_throughput_individual_avg.csv \
    tests/mps/analysis/stage2/05052025_nonDL_freq300_nodvfs_share_comb2_freqscale_throughput_individual_avg.csv \
...
```bash
python tests/mps/eval_baselines/mudi/curvefit/get_optimal_allocation.py --cutoff
```
...
```bash
python tests/mps/eval_baselines/mudi/curvefit/kneedle_curvefit.py \
  --input_csv tests/mps/freq_scaling/baseline_metrics/0506_FREQ300_baseline_metrics.csv \
...
```bash
python tests/mps/eval_baselines/gslice/gslice_allocator.py \
  --throughput_csv \
    tests/mps/analysis/stage2/05052025_freq300_share_comb2_freqscale_throughput_individual_avg.csv \
    tests/mps/analysis/stage2/05052025_nonDL_freq300_nodvfs_share_comb2_freqscale_throughput_individual_avg.csv \
  --caps_csv tests/mps/eval_baselines/mudi/curvefit/summary/single_freq300.csv \
  --arrival_csv tests/mps/freq_scaling/baseline_metrics/0506_FREQ300_baseline_metrics.csv \
  --n_comb 2 \
  --summary_name colocated_freq300.csv \
...
`muxflow/muxflow.py` combines four prerequisite data sources:
...
The paths are relative to `tests/mps/freq_scaling/dataset/`.
...
replace its historical `/home/bihhan/mlProfiler` prefix with
`/home/bing/PEACE-artifact`, and set `powercap_limit` and `power_epsilon` for
each run. Run it from its directory because its intermediate filenames are
relative:

```bash
cd /home/bing/PEACE-artifact/tests/mps/eval_baselines/muxflow
python muxflow.py
```
...
```bash
python tests/mps/eval_baselines/mudi/curvefit/kneedle_curvefit.py \
  --input_csv tests/mps/analysis/stage2/09152025_freq300_DL_comb3_share_comb3_freqscale_throughput_individual_avg.csv \
...
```bash
python tests/mps/eval_baselines/find_missing_comb3_threads.py
```
