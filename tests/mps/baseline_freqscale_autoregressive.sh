                ...
                for FREQ_SCALING in "${VALID_FREQS[@]}"; do
                    ...
                    if [ $LS_PERCENTAGE -eq 0 ]; then
                        ...
                        if [ "$LS_MODE" == "train" ]; then
                            ...
                            --batch_size ${LS_BATCH} --log_dir ../../tests/mps/$CURR_LOG_DIR --profile_nstep 100"  \
                            > $CURR_LOG_DIR/LS_${LS_TASK}_FREQ${FREQ_SCALING}_MPS${LS_PERCENTAGE}.log 2>&1 & pids+=($!)
                        fi
                    fi
                    ...
                    if [ "$BE_MODE" == "train" ]; then
                        ...
                        --batch_size 1 --log_dir ../../tests/mps/$CURR_LOG_DIR --output_length ${BE_OUTPUTLEN} --num_requests 1000000000 --profile_nstep 100"  \
