...
for RUN in $(seq 1 $RUNS); do 
    ...
    for ((i=0; i<${#BE_PERCENTAGES[@]}; i++)); do
        ...
        for TASK_MODEL in "${TASKS_MODELS[@]}"; do
            ...
            echo "TASK=$TASK, MODEL=$MODEL BE_EPOCH=$BE_EPOCH BATCH=$BE_OUTPUTLEN"
            CURR_LOG_DIR="$LOG_DIR/RUN${RUN}/${TASK}/BE_${MODEL}_batch${BE_OUTPUTLEN}"
            ...
            if [ $BE_PERCENTAGE -eq 0 ]; then
                ...
                if [ "$BE_MODE" == "train" ]; then
                    echo "BE_MODE=train"
                    echo "train of autorergressive model is not included"
                else
                    ...
                    --batch_size 1 --log_dir ../../tests/mps/$CURR_LOG_DIR --output_length ${BE_OUTPUTLEN} --num_requests 10000000 --profile_nstep 15"  \
