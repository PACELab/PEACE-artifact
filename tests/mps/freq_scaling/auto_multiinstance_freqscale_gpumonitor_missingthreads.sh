...
source config_gpumonitor_missingthreads.sh
...
# ------------------------------------------------------------------------------
# 4) MAIN LOOP OVER CSV ROWS (header-driven, N_COMBINATIONS-aware)
# ------------------------------------------------------------------------------
...
if (( powercap_col_index < 0 )); then
    echo "Missing PowerCap header in $CSV_FILE"
    exit 1
fi
...
while IFS=$'\t' read -r -a columns; do
    ...
    # Extract powercap by header index and compute threshold value
    ...
    for RUN in $(seq 1 $RUNS); do
        ...
        for SLEEP_TIME in "${SLEEP_TIMES[@]}"; do
            ...
            # Start GPU Power Monitor in the background
            mps_combination_underscore=$(echo ${mps_combination// /_})
            python GPUpowerMonitor.py \
                --logfile "$MPS_LOG_DIR/gpu_monitor_FREQ0_${mps_combination_underscore}_SLEEP${SLEEP_TIME}.log" \
