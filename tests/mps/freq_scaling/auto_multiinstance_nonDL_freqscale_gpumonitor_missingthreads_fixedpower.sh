...
while IFS=, read -ra columns; do
    ...
    for RUN in $(seq 1 $RUNS); do
        ...
        for SLEEP_TIME in "${SLEEP_TIMES[@]}"; do
            ...
            monitor_log="$MPS_LOG_DIR/gpu_monitor_FREQ0_${w1_percentage}_${w2_percentage}_SLEEP${SLEEP_TIME}.log"
            if [ -f "$monitor_log" ]; then
                rm "$monitor_log"
                echo "Deleted monitor log: $monitor_log"
            else
                echo "No monitor log found: $monitor_log"
            fi
            ...
            for (( i=0; i<${#workload_names[@]}; i++ )); do
                ...
                if [[ "${modes[$i]}" == "cuda_samples" ]]; then
                    ...
                    docker run --rm --name "w$((i+1))" \
                        ...
                        --batch_size ${batchsizes[$i]} --log_dir ../../tests/mps/multiinstance/$MPS_LOG_DIR --profile_nstep 999999999" \
