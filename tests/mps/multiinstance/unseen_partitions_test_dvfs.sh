...
istrain=$4
ispredict=$5
throughput_or_power_task=$6
rm_100partitions=$7
root_train_dataset_filename=$8
root_test_dataset_filename=$9
root_train_dataset_dir=${10}
root_test_dataset_dir=${11}
root_train_model_dir=${12}
outdir=${13}
...
echo "root_train_dataset_filename: $root_train_dataset_filename"
echo "root_test_dataset_filename: $root_test_dataset_filename"
echo "root_train_dataset_dir: $root_train_dataset_dir"
echo "root_test_dataset_dir: $root_test_dataset_dir"
echo "root_train_model_dir: $root_train_model_dir"
echo "outdir: $outdir"
...
EXCOL=""
#dataset location
full_train_data_path="$root_train_dataset_dir/$root_train_dataset_filename"
full_test_data_path="$root_test_dataset_dir/$root_test_dataset_filename"
#for splitting
n_combination="2"
...
root_train_dataset_dir="$root_train_dataset_dir/unseen_partition/$throughput_or_power_task"
root_test_dataset_dir="$root_test_dataset_dir/unseen_partition/$throughput_or_power_task"
...
then
    ...
    do
        ...
        do
            echo "seed $seed"
            train_dataset_dir="$root_train_dataset_dir/rand${seed}/$modeltype/$workload"
            test_dataset_dir="$root_test_dataset_dir/rand${seed}/$modeltype/$workload"
            outdir_rand="$root_train_model_dir/unseen_partition/${throughput_or_power_task}/rand${seed}/$modeltype/$workload"
            mkdir -p "$test_dataset_dir"
            mkdir -p "$outdir_rand"
            #if data is not generated, generate data
            if [ ! -f "$test_dataset_dir/$test_file" ] || [ ! -f "$test_dataset_dir/$train_file" ]
            then
                echo "generate testing data for $test_dataset_dir..."
                echo ""
                chmod +x "$test_dataset_dir"
                ...
                "$full_test_data_path" "$workload" "$test_dataset_dir" "$n_combination" "$rm_100partitions"
            ...
            echo "train model with $test_dataset_dir/$train_file..."
            echo "no training should have been done..."
...
#Predict throughput for different workloads
    ...
    do
        ...
        do
            ...
            train_dataset_dir="$root_train_dataset_dir/rand${seed}/$modeltype/$workload"
            test_dataset_dir="$root_test_dataset_dir/rand${seed}/$modeltype/$workload"
            model_dir="$root_train_model_dir/unseen_partition/${throughput_or_power_task}/rand${seed}/$modeltype/$workload"
            ...
            echo "using training dataset  $train_dataset_dir/$train_file"
            echo "using testing dataset  $test_dataset_dir/$test_file"
            #iterate  over the workloads
        
            
            python ../../../main.py  --test_file "$test_dataset_dir/$test_file" \
            --train_file  "$train_dataset_dir/$train_file" \
            --train_postprocess_file "$train_dataset_dir/X_train_postprocess_$label_policy.csv" \
            ...
            echo testfile=$test_dataset_dir/$test_file >  $outdir_pred/config.txt
            echo trainfile=$train_dataset_dir/$train_file >>  $outdir_pred/config.txt
